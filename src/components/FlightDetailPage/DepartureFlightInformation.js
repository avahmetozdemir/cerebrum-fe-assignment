...
function DepartureFlightInformation({ flight, where, airline }) {
  //const {  route, codeshares, publicFlightState, id } = flight;
  const config = {
    headers: {
  ...
  ///TODO- show departure date

  useEffect(() => {
    const fetchDestination = async () => {
      const response = await axios.get(
        `http://localhost:5000/public-flights/destinations/${destination}`,
        config
      );
